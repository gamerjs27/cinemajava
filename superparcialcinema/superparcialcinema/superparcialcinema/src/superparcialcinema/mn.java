...
class mn {
    ...
    private void crear_una_nueva_tarjeta() throws IOException {
        ...
        if (10 >= Integer.toString(cdltemp).length() && cdltemp != 0 && !i) {
            JOptionPane.showMessageDialog(null, "cedula valida", "cinemark", JOptionPane.ERROR_MESSAGE, icono);
            tj.creartarjetacinemark(cdltemp);
        } else {
            JOptionPane.showMessageDialog(null, "no se pudo crear intenta otravez", "error 23", JOptionPane.ERROR_MESSAGE, icono);
        }
    ...
    private void Crear_una_reserva() throws IOException {
        System.err.println("Crear_una_reserva" + " inicio correctamente");
        int sala = 0, puesto = 0, largo = 0;
        String sublargo;
        //pedir cedula
        ...
        if (tj.nuevareservatjv(cdltemp)) {
            ...
            sublargo = (JOptionPane.showInputDialog(null, "elige una fila a ala j"));
            if (null != sublargo) {
                switch (sublargo) {
                    case "a":
                        largo = 1;
    ...
    private void Eliminar_la_reserva_actual() throws IOException {
    ...
    private void Pagar_una_reserva_en_efectivo() throws IOException {
        int largo = 0;
        System.err.println("Pagar_una_reserva_en_efectivo" + " inicio correctamente");
        int sala = Integer.parseInt(JOptionPane.showInputDialog(null, "eliga una pelicula /n"
                + "1=avartar /n"
                + "2=Scott Pilgrim vs. los ex de la chica de sus sueños /n"
                + "3=El recuerdo de Marnie"));
        int puesto = Integer.parseInt(JOptionPane.showInputDialog(null, "elige un puesto del un al 5"));
        ...
        //mostrar costo 
        int costo = tj.comprarpuestoefectivov(largo);
        if (sl.estareservada(sala, puesto, largo)) {
            if (1 == JOptionPane.showConfirmDialog(null, "desea recargar pagar", "recarga cinemark", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE)) {
                tj.comprarpuestoefectivo(largo);
    ...
    private void Visualizar_el_dinero_en_caja() {//terminado
